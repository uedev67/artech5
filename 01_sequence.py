...
def run_sequence_1(command_queue: Queue):
    # ... (기존 코드와 동일, sleep 시간만 0.2초로 수정)
    ser = None
    port = 'COM3'
    baudrate = 9600
    try:
        ...
        return {"target_age": target_age, "gender": survey_result.get("gender"), "theme": survey_result.get("theme")}
    except serial.SerialException as e:
...
def run_artech5():
    ...
    # --- [핵심 수정] 모든 자식 프로세스를 추적하기 위한 리스트 ---
    active_processes = []

    try:
        # --- 1단계: 통신 가능한 블랙스크린 시작 ---
        command_queue = multiprocessing.Queue()
        black_screen_proc = Process(target=run_black_screen_process, args=(command_queue,), daemon=True)
        black_screen_proc.start()
        active_processes.append(black_screen_proc) # 추적 리스트에 추가
        time.sleep(0.5)

        sequence_1_result = run_sequence_1(command_queue)
        if not sequence_1_result:
            print("❌ [SERVER] 시퀀스 1 실행에 실패하여 전체 프로그램을 종료합니다.")
            return

        print("[SYSTEM] 캡처를 위해 검은 화면을 일시적으로 숨깁니다.")
        command_queue.put('hide')
        time.sleep(0.5)
        face1 = capture()
        ...
        print("[SYSTEM] 화면 관리 방식 전환. 통신 블랙스크린을 종료합니다.")
        black_screen_proc.terminate()
        black_screen_proc.join()
        active_processes.remove(black_screen_proc) # 추적 리스트에서 제거

        # --- 2단계: 단순 블랙스크린을 배경으로 시퀀스 2 진행 ---
        print("[SYSTEM] 시퀀스 2를 위해 단순 블랙스크린을 시작합니다.")
        black_screen_simple_proc = Process(target=run_black_screen_simple, daemon=True)
        black_screen_simple_proc.start()
        active_processes.append(black_screen_simple_proc) # 추적 리스트에 추가
        time.sleep(0.5)
        ...
        try:
            ...
            video_process.start()
            active_processes.append(video_process) # 추적 리스트에 추가
            result = result_queue.get() 
            video_process.join()
            active_processes.remove(video_process) # 추적 리스트에서 제거
        ...
        # --- [핵심 수정] 프로그램 종료 시 살아있는 모든 자식 프로세스 강제 종료 ---
