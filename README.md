...
[관객 입장 전 세팅]
밀실에 먼저 들어가서 B1, 빛 들어온 버튼, 문닫힘 버튼 총 3개 버튼을 눌러서 리셋해줍니다.

밀실 안의 모니터에서 파이썬 파일 실행버튼을 누릅니다. 검은 화면이 뜨면 밀실을 나와줍니다.

밖에서 기다리는 관객에게 노트북으로 설문을 시킵니다.

관객이 설문을 마치면 세팅이 완료됩니다. 관객이 밀실에 들어가면 문닫힘버튼만 불이 들어오며 이 버튼을 누르면 콘텐츠가 자동 진행됩니다.
